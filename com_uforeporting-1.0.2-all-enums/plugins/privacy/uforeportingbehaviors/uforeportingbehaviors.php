...
/**
 * Privacy plugin managing Joomla user behavior data
 *
 * @since  1.0.0
 */
...
{
	/**
	 * Processes an export request for behavior data
	 *
	 * This event will collect data for the behavior table
	 *
	...
	public function onPrivacyExportRequest(RequestTable $request, User $user = null)
	{
		...
		$domains   = array();
		$domain    = $this->createDomain('user_behavior', 'joomla_user_behavior_data');
		$domains[] = $domain;

		$query = $this->db->getQuery(true)
			->select('*')
			->from($this->db->quoteName('#__ommp_behaviors'))
		...
		$domains[] = $this->createCustomFieldsDomain('com_uforeporting.behavior', $items);
	 ...
	 *
	 * This event will pseudoanonymise the behavior
	 *
	 * @param   RequestTable  $request  The request record being processed
	 * @param   User          $user     The user account associated with this request if available
	 *
	...
	public function onPrivacyRemoveData(RequestTable $request, User $user = null)
	{
		...
		$query->clear()
			->delete($db->quoteName('#__ommp_behaviors'))
			->where($this->db->quoteName('created_by') . ' = ' . (int) $user->id);
