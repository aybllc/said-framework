...
/**
 * Privacy plugin managing Joomla user vehicle data
 *
 * @since  1.0.0
 */
...
{
	/**
	 * Processes an export request for vehicle data
	 *
	 * This event will collect data for the vehicle table
	 *
	...
	public function onPrivacyExportRequest(RequestTable $request, User $user = null)
	{
		...
		$domains   = array();
		$domain    = $this->createDomain('user_vehicle', 'joomla_user_vehicle_data');
		$domains[] = $domain;

		$query = $this->db->getQuery(true)
			->select('*')
			->from($this->db->quoteName('#__ommp_observer_vehicle'))
		...
		$domains[] = $this->createCustomFieldsDomain('com_uforeporting.vehicle', $items);
	 ...
	 *
	 * This event will pseudoanonymise the vehicle
	 *
	 * @param   RequestTable  $request  The request record being processed
	 * @param   User          $user     The user account associated with this request if available
	 *
	...
	public function onPrivacyRemoveData(RequestTable $request, User $user = null)
	{
		...
		$query->clear()
			->delete($db->quoteName('#__ommp_observer_vehicle'))
			->where($this->db->quoteName('created_by') . ' = ' . (int) $user->id);
