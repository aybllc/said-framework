...
/**
 * Privacy plugin managing Joomla user kinematic data
 *
 * @since  1.0.0
 */
...
{
	/**
	 * Processes an export request for kinematic data
	 *
	 * This event will collect data for the kinematic table
	 *
	...
	public function onPrivacyExportRequest(RequestTable $request, User $user = null)
	{
		...
		$domains   = array();
		$domain    = $this->createDomain('user_kinematic', 'joomla_user_kinematic_data');
		$domains[] = $domain;

		$query = $this->db->getQuery(true)
			->select('*')
			->from($this->db->quoteName('#__ommp_observer_kinematics'))
		...
		$domains[] = $this->createCustomFieldsDomain('com_uforeporting.kinematic', $items);
	 ...
	 *
	 * This event will pseudoanonymise the kinematic
	 *
	 * @param   RequestTable  $request  The request record being processed
	 * @param   User          $user     The user account associated with this request if available
	 *
	...
	public function onPrivacyRemoveData(RequestTable $request, User $user = null)
	{
		...
		$query->clear()
			->delete($db->quoteName('#__ommp_observer_kinematics'))
			->where($this->db->quoteName('created_by') . ' = ' . (int) $user->id);
