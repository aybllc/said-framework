...
/**
 * Privacy plugin managing Joomla user pair data
 *
 * @since  1.0.0
 */
...
{
	/**
	 * Processes an export request for pair data
	 *
	 * This event will collect data for the pair table
	 *
	...
	public function onPrivacyExportRequest(RequestTable $request, User $user = null)
	{
		...
		$domains   = array();
		$domain    = $this->createDomain('user_pair', 'joomla_user_pair_data');
		$domains[] = $domain;

		$query = $this->db->getQuery(true)
			->select('*')
			->from($this->db->quoteName('#__ommp_invalid_pairs'))
		...
		$domains[] = $this->createCustomFieldsDomain('com_uforeporting.pair', $items);
	 ...
	 *
	 * This event will pseudoanonymise the pair
	 *
	 * @param   RequestTable  $request  The request record being processed
	 * @param   User          $user     The user account associated with this request if available
	 *
	...
	public function onPrivacyRemoveData(RequestTable $request, User $user = null)
	{
		...
		$query->clear()
			->delete($db->quoteName('#__ommp_invalid_pairs'))
			->where($this->db->quoteName('created_by') . ' = ' . (int) $user->id);
