...
namespace Uforresporting\Component\Uforeporting\Administrator\Model;
// No direct access.
 ...
 * @since  1.0.0
 */
class KinematicModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_UFOREPORTING';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_uforeporting.kinematic';
	...
	public function userIDItem($id)
	{
		try
		{
			...
			$query->select("id")
						->from($db->quoteName('#__ommp_observer_kinematics'))
	 ...
	 * @since   1.0.0
	 */
	public function getTable($type = 'Kinematic', $prefix = 'Administrator', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_uforeporting.edit.kinematic.data', array());
	...
	/**
	 * Method to duplicate an Kinematic
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_uforeporting'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__ommp_observer_kinematics');
				$max             = $db->loadResult();
