...
class EnvironmentModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_uforeporting.environment';
	...
	public function userIDItem($id)
	{
		try
		{
			...
			$query->select("id")
						->from($db->quoteName('#__ommp_environment'))
	 ...
	 * @since   1.0.0
	 */
	public function getTable($type = 'Environment', $prefix = 'Administrator', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_uforeporting.edit.environment.data', array());
	...
	/**
	 * Method to duplicate an Environment
	 *
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__ommp_environment');
				$max             = $db->loadResult();
