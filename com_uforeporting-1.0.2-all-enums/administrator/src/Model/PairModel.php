 ...
 * @since  1.0.0
 */
class PairModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_uforeporting.pair';
	...
	public function userIDItem($id)
	{
		try
		{
			...
			$query->select("id")
						->from($db->quoteName('#__ommp_invalid_pairs'))
	 ...
	 * @since   1.0.0
	 */
	public function getTable($type = 'Pair', $prefix = 'Administrator', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_uforeporting.edit.pair.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__ommp_invalid_pairs');
				$max             = $db->loadResult();
