...
class VehicleModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_uforeporting.vehicle';
	...
	public function userIDItem($id)
	{
		try
		{
			...
			$query->select("id")
						->from($db->quoteName('#__ommp_observer_vehicle'))
	 ...
	 * @since   1.0.0
	 */
	public function getTable($type = 'Vehicle', $prefix = 'Administrator', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_uforeporting.edit.vehicle.data', array());
	...
	/**
	 * Method to duplicate an Vehicle
	 *
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__ommp_observer_vehicle');
				$max             = $db->loadResult();
