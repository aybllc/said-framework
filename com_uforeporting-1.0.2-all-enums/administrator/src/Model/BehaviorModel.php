 ...
 * @since  1.0.0
 */
class BehaviorModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_uforeporting.behavior';
	...
	public function userIDItem($id)
	{
		try
		{
			...
			$query->select("id")
						->from($db->quoteName('#__ommp_behaviors'))
	 ...
	 * @since   1.0.0
	 */
	public function getTable($type = 'Behavior', $prefix = 'Administrator', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_uforeporting.edit.behavior.data', array());
	...
	/**
	 * Method to duplicate an Behavior
	 *
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__ommp_behaviors');
				$max             = $db->loadResult();
